...
We also expected the additional covariates in the full logistic model to increase its viability as a troll predictor (Table 2). This was evident as the error rate dropped  $`r round(log1_err_rate, 3)`$ to $`r round(log2_err_rate, 3)`$, while   sensitivity increase (from $`r round(log1_sens, 3)`$ to $`r round(log2_sens, 3)`$), as well as specificity (from $`r round(log1_spec, 3)`$ to $`r round(log2_spec, 3)`$), and AUC (from $`r round(log1_auc, 3)`$ to $`r round(log2_auc, 3)`$) in the two models (Table 2). 


Surprisingly, many of the covaraites did not make the final model in the pruned classification tree model. These included the anger score, emoji score, number of individuals they are following etc. This could be because of a lack of relationship between these variables and troll status, or it could be due to the lack of data to identify such nuanced features.
