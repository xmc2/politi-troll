...
logistic regrtession  `r round(log1_err_rate, 3)` to `r round(log2_err_rate, 3)`, while increasing the sensitivity (from `r round(log1_sens, 3)` to `r round(log2_sens, 3)`), specificty (from `r round(log1_spec, 3)` to `r round(log2_spec, 3)`), and the AUC (from `r round(log1_auc, 3)` to `r round(log2_auc, 3)`).
...
Surprisingly, many of the covaraites did not make the final model in either the logisic or classification tree case. These included the anger score, bot users, emoji score, etc. This could be because of a lack of relationship between these variables and troll status, or it could be due to the lack of data to identify such nuanced features.
